...
struct DatagramHeaderFormat
{
	...
	void Serialize(DatagramHeaderFormat *dhf, RakNet::BitStream *b)
	{
		// Not endian safe
		...
		{
			b->Write(true);
			b->Write(dhf->hasBAndAS);
			b->AlignWriteToByteBoundary();
#if INCLUDE_TIMESTAMP_WITH_DATAGRAMS==1
			RakNet::TimeMS timeMSLow=(RakNet::TimeMS) sourceSystemTime&0xFFFFFFFF; b->Write(dhf->timeMSLow);
#endif
			if (dhf->hasBAndAS)
			{
				//		b->Write(B);
		...
		else
		{
			b->Write(false);
			b->Write(false);
			b->Write(dhf->isPacketPair);
			b->Write(dhf->isContinuousSend);
			b->Write(dhf->needsBAndAs);
			b->AlignWriteToByteBoundary();
#if INCLUDE_TIMESTAMP_WITH_DATAGRAMS==1
			RakNet::TimeMS timeMSLow=(RakNet::TimeMS) sourceSystemTime&0xFFFFFFFF; b->Write(dhf->timeMSLow);
#endif
			b->Write(dhf->datagramNumber);
		}
	}
	void Deserialize(DatagramHeaderFormat *dhf, RakNet::BitStream *b)
	{
		// Not endian safe
		...
		else
		{
			...
			{
				b->Read(dhf->isPacketPair);
				b->Read(dhf->isContinuousSend);
				b->Read(dhf->needsBAndAs);
				b->AlignReadToByteBoundary();
#if INCLUDE_TIMESTAMP_WITH_DATAGRAMS==1
				RakNet::TimeMS timeMS; b->Read(timeMS); sourceSystemTime=(CCTimeType) timeMS;
#endif
				b->Read(dhf->datagramNumber);
			}
		}
	}
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
	...
	if (NAKs.Size()>0)
	{
		...
		Serialize(&dhfNAK, &updateBitStream);
